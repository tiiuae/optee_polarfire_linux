...
#define MSSPLL_POSTDIV_SHIFT	0x18u
...
/*
 * the only two supported reference clock frequencies for the mpfs
 * are 100 and 125 MHz, as the rtc reference is required to be 1 MHz
 * it then only needs to have divider table entries corresponding for
...
static int mpfs_cfg_clk_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long prate)
{
	...
	divider_setting = divider_get_val(rate, prate, cfg->table, cfg->width, cfg->flags);
 ...
 * - CLK_FICx: these provide the microprocessor side clocks to the "fic" clock domain crossers
 *   which provide the fabric interface. disabling them would cause the FPGA fabric to go into
 *   reset
 */
