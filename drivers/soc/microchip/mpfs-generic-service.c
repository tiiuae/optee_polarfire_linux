					  ...
					  size_t len, loff_t *offset)
{
	...
	if(generic_service_priv->response != NULL)
		mpfs_generic_service_free_message_structs();

	response = devm_kmalloc(generic_service_priv->dev, sizeof(struct mpfs_mss_response), GFP_KERNEL);
	if (!response) {
		devm_kfree(generic_service_priv->dev, cmd_data);
		return -ENOMEM;
	}
	...
	response->resp_size = ALIGN((u16)(*(cmd_data + 3)) + ((u16)(*(cmd_data + 4) << 8)), (4U));
	response->resp_msg  = (u32 *)devm_kzalloc(generic_service_priv->dev, response->resp_size*sizeof(u8), GFP_KERNEL);
	...
	struct mpfs_mss_msg msg = {
		.cmd_opcode = *(cmd_data), // do i need to check what the opcode is?
		.cmd_data_size = (u16)(*(cmd_data + 1)) + (u16)(*(cmd_data + 2) << 8),
		.response = generic_service_priv->response,
		.cmd_data = cmd_data+9,
		.mbox_offset = (u16)(*(cmd_data + 5)) + ((u16)(*(cmd_data + 6) << 8)),
		.resp_offset = (u16)(*(cmd_data + 7)) + ((u16)(*(cmd_data + 8) << 8))
	};

	if (msg.cmd_data_size != len-9)
		return -EINVAL;
...
{
	u8 *buffer, *bufferp, *response_msg;
	int i, ret;
